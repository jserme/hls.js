...
class AudioTrackController extends BasePlaylistController {
  ...
  private selectInitialAudioTrack (): void {
    const audioTracks = this.tracksInGroup;
    console.assert(audioTracks.length, 'Initial audio track should be selected when tracks are known');

    const currentAudioTrackName = audioTracks[this.trackId]?.name; // _tracks

    // Pre-select default tracks if there are any
    let tracks = audioTracks;
    if (this.selectDefaultTrack) {
      const defaultTracks = audioTracks.filter((track) => track.default);
    ...
    const traverseTracks = (name?: string): boolean => {
      // Select track with right group ID
      return tracks.some((track): boolean => {
    ...
    let trackFound = traverseTracks(currentAudioTrackName);

    if (!trackFound) {
      trackFound = traverseTracks();
    }
